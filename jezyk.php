...
function wskaznik($strona, $liczba_stron)
{
    ...
    if ($strona > 1) {
		$wynik .= " <li class='page-item'><a class='page-link' href='jezyk.php?strona=1'><i class='material-icons'>first_page</i></a></li> ";
    } else {
    ...
    $poprzednia = $strona - 1;
    if ($poprzednia > 0) {
        $wynik .= " <li class='page-item'><a class='page-link' href='jezyk.php?strona=$poprzednia'><i class='material-icons'>navigate_before</i></a></li> ";
    } else {
    ...
    if ($nastepna <= $liczba_stron) {
        $wynik .= " <li class='page-item'><a class='page-link' href='jezyk.php?strona=$nastepna'><i class='material-icons'>navigate_next</i></a></li> ";
    } else {
    ...
    if ($strona < $liczba_stron) {
        $wynik .= " <li class='page-item'><a class='page-link' href='jezyk.php?strona=$liczba_stron'><i class='material-icons'>last_page</i></a></li> ";
    } else {
...
?>
		<div class="page-header">
			<h1>Język <span></span></h1>
		</div>
		
<?php
	$tab_danych = array(); //zainicjowanie tablicy bez wartosci
	szukaj_danych4('jezyk', 'wejscia', "jezyk");
?>
...
//STRONNICOWANIE
	...
	$stmt = $db->query("SELECT * FROM jezyk");
	...
	<div class="table-responsive">
		...
		<tr>
			<th>język</th> <th>wizyty</th> <th>wykres</th> <th>usuń</th>
		</tr>';

		szukaj_naj_godziny('jezyk');
		
		$stmt = $db->query("SELECT * FROM `jezyk` ORDER BY `jezyk`.`wejscia` DESC LIMIT $start ,$rekordow_na_stronie");
			...
			while($wiersz = $stmt->fetch(PDO::FETCH_ASSOC)){
				...
				echo'
				<tr>
					<td class="text-muted">'.$jezyk.'</td> <td class="text-muted">'.$wejscia.'</td> <td><div class="row_slupki_poziom ttt" style="width: '.$szer.'px;" data-toggle="tooltip" data-placement="right" title="'.$wejscia.' wiz."></span></div></td> <td><button type="button" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#szczegoly'.$id.'" title="Usuń"><i class="material-icons">delete_forever</i></button></td> 
				</tr>';
						...
						<div class="modal-dialog modal-lg">
							<div class="modal-content">
								...
								<div class="modal-body">
									...
									<form action="jezyk.php" method="post" class="form-horizontal">
										<ul class="list-group">								
											<li class="list-group-item">
												<div class="row">
													<div class="col-md-3">nazwa</div>						
													<div class="col-md-9"><b>'.$jezyk.'</b></div>						
												</div>
											</li>									
										</ul>
								</div>
								...
								<div class="modal-footer">
										...
										<input type="hidden" value="'.$jezyk.'" name="jezyk">
								
										<button type="button" class="btn btn-secondary" data-dismiss="modal">Nie</button>
										<button type="submit" class="btn btn-success" name="wyslij_5" title="tak usuń">Tak</button>
									</form>
								</div>
