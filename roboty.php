...
function wskaznik($strona, $liczba_stron)
{
    ...
    if ($strona > 1) {
		$wynik .= " <li class='page-item'><a class='page-link' href='roboty.php?strona=1'><i class='material-icons'>first_page</i></a></li> ";
    } else {
    ...
    $poprzednia = $strona - 1;
    if ($poprzednia > 0) {
        $wynik .= " <li class='page-item'><a class='page-link' href='roboty.php?strona=$poprzednia'><i class='material-icons'>navigate_before</i></a></li> ";
    } else {
    ...
    if ($nastepna <= $liczba_stron) {
        $wynik .= " <li class='page-item'><a class='page-link' href='roboty.php?strona=$nastepna'><i class='material-icons'>navigate_next</i></a></li> ";
    } else {
    ...
    if ($strona < $liczba_stron) {
        $wynik .= " <li class='page-item'><a class='page-link' href='roboty.php?strona=$liczba_stron'><i class='material-icons'>last_page</i></a></li> ";
    } else {
...
//STRONNICOWANIE
	...
	$stmt = $db->query("SELECT * FROM roboty");
	...
	<div class="table-responsive">
		...
		<tr>
			<th>roboty</th> <th>odsłony</th> <th>wykres</th> <th>data</th> <th>user agent</th> <th>usuń</th>
		</tr>';

		szukaj_naj_godziny('roboty');
		
		$stmt = $db->query("SELECT * FROM `roboty` ORDER BY `roboty`.`wejscia` DESC LIMIT $start ,$rekordow_na_stronie");

			while($wiersz = $stmt->fetch(PDO::FETCH_ASSOC)){
				
				$id 			= $wiersz['id'];
				$roboty 		= $wiersz['roboty'];
				...
				echo'
				<tr>
					<td class="text-muted">'.$roboty.'</td> <td class="text-muted">'.$wejscia.'</td> <td><div class="row_slupki_poziom ttt" style="width: '.$szer.'px;" data-toggle="tooltip" data-placement="right" title="'.$wejscia.' ods."></div></td> <td class="text-muted">'.$data_utw.'</td> <td class="text-muted">'.$ua.'</td> <td><button type="button" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#szczegoly'.$id.'" title="Usuń"><i class="material-icons">delete_forever</i></button></td> 
				</tr>';
						...
						<div class="modal-dialog modal-lg">
							<div class="modal-content">
								...
								<div class="modal-body">
									...
									<form action="roboty.php" method="post" class="form-horizontal">
										<ul class="list-group">								
											<li class="list-group-item">
												<div class="row">
													<div class="col-md-3">nazwa</div>						
													<div class="col-md-9"><b>'.$roboty.'</b></div>						
												</div>
											</li>									
										</ul>
								</div>
								...
								<div class="modal-footer">
										...
										<input type="hidden" value="'.$roboty.'" name="roboty">
...
<div class="collapse" id="collapseExample">
	<div class="card card-body">
		Dane zawarte w powyższej tabeli wskazują nazwę robotów internetowych które trafiły na stronę mnitorowaną, są zliczane odsłony, zapisany też jest ciąg User Agenta, dodano też możliwość usunięcia pojedynczych rekordów.
	</div>
</div>
