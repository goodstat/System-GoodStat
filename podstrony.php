...
function wskaznik($strona, $liczba_stron)
{
    ...
    if ($strona > 1) {
		$wynik .= " <li class='page-item'><a class='page-link' href='podstrony.php?strona=1'><i class='material-icons'>first_page</i></a></li> ";
    } else {
    ...
    $poprzednia = $strona - 1;
    if ($poprzednia > 0) {
        $wynik .= " <li class='page-item'><a class='page-link' href='podstrony.php?strona=$poprzednia'><i class='material-icons'>navigate_before</i></a></li> ";
    } else {
    ...
    if ($nastepna <= $liczba_stron) {
        $wynik .= " <li class='page-item'><a class='page-link' href='podstrony.php?strona=$nastepna'><i class='material-icons'>navigate_next</i></a></li> ";
    } else {
    ...
    if ($strona < $liczba_stron) {
        $wynik .= " <li class='page-item'><a class='page-link' href='podstrony.php?strona=$liczba_stron'><i class='material-icons'>last_page</i></a></li> ";
    } else {
...
//STRONNICOWANIE
	...
	$stmt = $db->query("SELECT * FROM podstrony");
	...
	<div class="table-responsive">
		...
		$stmt = $db->query("SELECT * FROM `podstrony` ORDER BY `podstrony`.`wejscia` DESC LIMIT $start ,$rekordow_na_stronie");
...
<div class="collapse" id="collapseExample">
	<div class="card card-body">
		Dane zawarte w powyższej tabeli wskazują odsłony poszczególnych podstron na stronie monitorowanej. Dodano też możliwość usunięcia pojedynczych rekordów.
	</div>
</div>
